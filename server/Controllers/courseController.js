...
export const addLectureToCourseById = async (req, res, next) => {
  ...
  if (!course) {
    ...
    const lectureData = {               //let lectureData = {}
        title,
        description,
        lecture:{}
  ...
  if (req.file) {
    ...
    /*  // Empty the uploads directory without deleting the uploads directory(mtlb uplodas folder ke andar jitne bhi files hai unko delete krdo bina uplodas folder ko delete kiye)
      for (const file of await fs.readdir('uploads/')) {
        await fs.unlink(path.join('uploads/', file));
      }
        */
      await fs.unlink(req.file.path); // Remove the currently uploaded file from local storage (mtlb from uploads folder) without deleting the entire uploads folder and without deleting other files in the uploads folder
      // Send the error message
      return next(
        new AppError(
          JSON.stringify(error) || 'File not uploaded, please try again',
 ...
 * @ACCESS Private (Admin only)
 */
export const removeLectureFromCourse = async function(req, res, next)  {
...
export const updateCourseById = async function(req, res, next) {
  ...
  const course = await Course.findByIdAndUpdate(
    ...
    {
     // new: true, // This will return the updated course instead of the old one
