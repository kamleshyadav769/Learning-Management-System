 ...
 */
export const buySubscription = async function(req, res, next) {
  try{
  ...
  // If subscription is exists then initialize the subscription object in user schema if it is not already there
if (!user.subscription) {
 ...
 // return next(new AppError('Unable to subscribe, please try again later',500));
  ...
  console.log("🔥 ERROR DESCRIPTION:", e?.error?.description);
  console.log("🔥 ERROR MESSAGE:", e?.message);

  return next(
    new AppError(
      e?.error?.description || e?.message || "Subscription failed",
      500
    )
  );
}
 ...
 * @ACCESS Private (Logged in user only)
 */
export const verifySubscription = async function(req, res, next) {
  try{
  const { id } = req.user;
  ...
  return next(new AppError('Unable to verify payment, please try again later',500));
}
};
 ...
 * @ACCESS Private (Logged in user only)
 */
export const cancelSubscription = async function(req, res, next) {
  try{ 
  const { id } = req.user;
 ...
 try {
  ...
  return next(new AppError('Unable to cancel subscription, please try again later',500));
}
};
 ...
 * @ACCESS Public
 */
export const getRazorpayApiKey = async function (req, res, next) {
  try{
  res.status(200).json({
    success: true,
  ...
  return next(new AppError('Unable to get razorpay key, please try again later',500));
}
};
 ...
 */
export const allPayments = async function(req, res, next) {
  try{
  ...
  return next(new AppError('Unable to get payments, please try again later',500));
}
};